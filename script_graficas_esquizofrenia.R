setwd('/Users/paula/Documents/Duponte/graficas_r')
library(dplyr)
library(tidyr)
...
library(viridis)
install.packages
archivo= ('/Users/paula/Documents/Duponte/graficas_r/scopus_TRASTORNOS.csv')
...
datos_df$paises=sub("French Polynesia", "France", datos_df$paises) 
datos_df$paises=sub("North Carolina|Colorado|California|Virginia|Massachusetts|Minnesota|Houston", "United States", datos_df$paises) 

palabras_no_deseadas=c("Cardiology", "Science")
datos_df=datos_df %>%
  mutate(paises = str_replace_all(paises, paste(palabras_no_deseadas, collapse = "|"), ""))


# Eliminar los términos de la columna 'paises'
#datos_df=datos_df %>% #Esto aplica una transformación al dataframe datos_df
  #mutate(paises = str_replace_all(paises, paste(palabras_no_deseadas, collapse = "|"), ""))
#paste(palabras_no_deseadas, collapse = "|"): Este crea una expresión regular que une múltiples palabras o términos no deseados, separándolos con el símbolo | (OR lógico en regex).
#"": Reemplaza los términos encontrados con una cadena vacía, eliminándolos.
...
edges_count=edges_count[!apply(edges_count == "", 1, any), ]
...
#layout=norm_coords(layout, ymin = -2, ymax = 2, xmin = -2, xmax = 2)*2
...
png("/Users/paula/Documents/Duponte/graficas_r/trastornos_grafo_scopus.png", width = 3000, height = 2000, res = 300)  # Ajusta width, height y res según tus necesidades
...
png("/Users/paula/Documents/Duponte/graficas_r/trastornos_produccion_paises.png", width = 3500, height = 4500, res = 300)  # Ajusta width, height y res según tus necesidades
df_summary %>% 
  #filter(Production > 1) %>% 
